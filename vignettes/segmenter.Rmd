...
context. `segmenter` wraps the *Java* modules to call *ChromHMM* and captures the
...
`segmenter` provides additional tools to test, select and visualize the models.
...
This is a quick example of using `segmenter` to call the Java modules and will be
...
The main function in `segmenter` is called `learn_model`. This wraps the the Java
