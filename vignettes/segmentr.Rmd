...
context. `segmentr` wraps the *Java* modules to call *ChromHMM* and captures the
output in an `S4` object. This allows for iterating with different parameters,
which are given in *R* syntax. Capturing the output in *R* makes it easier to
work with the results and to integrate them in downstream analyses. Finally, 
`segmentr` provides additional tools to test, select and visualize the models.
...
The goal of the `segmentr` package is to
...
This is a quick example of using `segmentr` to call the Java modules and will be
followed by a detailed description in the following sections.
...
The Java modules and example files are bundled with `segmentr`. We can locate
...
above as well as some of the tools provided in `segmentr` to explore the 
resulting chromatin models.

# Segmentation analysis using `segmentr`
...
The main function in `segmentr` is called `learn_model`. This wraps the the Java
module that learns a chromatin segmentation model of a given number of states.
In addition to the input files explained before the function takes the desired
number of stats, `numstates` and the information that were used to generate the
binarized files. Those are the names of the genome `assembly`, the type of 
`annotation`, the `binsize` and the names of `cells` or conditions.
...
list of `segmentation` objects. `segmentr` contain functions to do basic 
...
- `GeneRegionTrack` to show the exson, intron and transcripts of the target gene
